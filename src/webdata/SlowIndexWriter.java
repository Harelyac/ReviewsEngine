...
public class SlowIndexWriter {
    ...
    private void mergeInvertedIndexes(String dir){
        ...
        int remainder1 = wordsTypeStrings.get(0).length() % 4;
        int remainder2 = wordsTypeStrings.get(1).length() % 4;

        // ptr location of heads words on each block
        int ptr1 = 0 , ptr2 = 0;

        // index for traversing each input blocks
        ...
        // run on both strings in block steps manner until we reach end of one string
        while (ptr1 != (size1 - remainder1) || ptr2 != (size2 - remainder2)){
            int startBlock1 = wordsTypeTables.get(0).get(ptr1).get("term_ptr") + 1;

            int startNextBlcok1 = wordsTypeStrings.get(0).length();
            // check if we reach last block
            if (ptr1 + 4 < size1 - 1){
                startNextBlcok1 = wordsTypeTables.get(0).get(ptr1 + 4).get("term_ptr") + 1;
            }

            int startBlock2 = wordsTypeTables.get(1).get(ptr2).get("term_ptr") + 1;

            int startNextBlcok2 = wordsTypeStrings.get(1).length();
            // check if we reach last block
            if (ptr1 + 4 < size2 - 1){
                startNextBlcok2 = wordsTypeTables.get(1).get(ptr2 + 4).get("term_ptr") + 1;
            }
            ...
            String[] block_words1 = block1.split("([@]+)|([*]+)|([|]+)");
            String[] block_words2 = block2.split("([@]+)|([*]+)|([|]+)");


            int result = (block_words1[0] + block_words1[index1]).compareTo(block_words2[0] + block_words2[index2]);

            List<String> block = null;
            Map<String, Long> row = null;
            // init new block
            block = new ArrayList<>();

            // traversing both blocks
            while (block.size() != 4)
            {
                if (index1 == 5){
                    ptr1 += 4;
                    index1 = index1 % (block_words1.length - 1);
                }

                if (index2 == 5){
                    ptr2 += 4;
                    index2 = index2 % (block_words2.length - 1);
                }


                result = (block_words1[0] + block_words1[index1]).compareTo(block_words2[0] + block_words2[index2]);

                // words are equal
                // here we sum freqs of each table
                // we create row with new term_ptr of output str and new reviewId_ptr and new freqs_ptr
                if (result == 0){
                    block.add(block_words1[0] + block_words1[index1]);
                    row = new HashMap<>();
                    row.put("total_freqs", (long)wordsTypeTables.get(0).get(ptr1 + index1).get("total_freqs") + wordsTypeTables.get(1).get(ptr2 + index2).get("total_freqs"));
                    ...
                    long startNextReviewsID1 = 0;
                    if (ptr1 + index1 < wordsTypeTables.get(0).size() - 1)
                    {
                    ...
                    long startNextReviewsID2 = 0;
                    if (ptr2 + index2 < wordsTypeTables.get(1).size() - 1)
                    {
                    ...
                    try {
                        ...
                        if (startNextReviewsID2 == 0){
                            startNextReviewsID2 = file1.length();
                        }
                ...
                // we create row with new term_ptr of output str and new reviewId_ptr and new freqs_ptr
                    ...
                    // put word from table 1 in output table
                    ...
                    if (ptr1 + index1 < wordsTypeTables.get(0).size() - 1)
                    {
                        System.out.println(wordsTypeTables.get(0).size());
                ...
                // we create row with new term_ptr of output str and new reviewId_ptr and new freqs_ptr
                    ...
                    block.add(block_words2[0] + block_words2[index2]);
                    row = new HashMap<>();
                    row.put("total_freqs", (long)wordsTypeTables.get(1).get(ptr2 + index2).get("total_freqs"));
                    // put word from table 2 in output table
                ...
                // write to output files with numbering of the chunknumber++
                // here the output block is full
            }
